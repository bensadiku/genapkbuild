...
fn extract_multiple_libs() {
    ...
    base.set_input("tests/data/x86_multiple_so.apk");
    base.set_name("");
    base.set_extract_so(true);
    base.set_has_default_architecture(true);
    base.set_default_architectures(vec!["x86".into()]);

    let ret = base.build().generate();
    assert_eq!(ret, 0);
...
fn non_extract_multiple_arch_libs() {
    ...
    mk.set_extract_so(false);
    let ret = mk.generate();
    assert_eq!(ret, 0);    let libhello_jnicallback_arm64_v8a = "@lib/arm64-v8a/libhello-jnicallback.so";
    let libhello_jnicallback_armeabi_v7a = "@lib/armeabi-v7a/libhello-jnicallback.so";
    let libhello_jnicallback_x86 = "@lib/x86/libhello-jnicallback.so";
    let libhello_jnicallback_x86_64 = "@lib/x86_64/libhello-jnicallback.so";
